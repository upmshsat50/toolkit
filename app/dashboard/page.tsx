  ...
  return (
    <>
      <section className="hero">
        ...
        <p>
          The Community Health Toolkit is a living digital resource for community clerks,
          interns, faculty, and preceptors—bringing together community profiles, projects,
          handovers, forms, learning resources, and institutional memory.
        </p>
        <div className="hero-actions">
          <Link className="button button-primary" href="/rotation">
            Open my rotation <Icon name="arrow" />
          </Link>
      ...
      <div className="grid-2">
        <section className="panel panel-pad">
          ...
          <div className="callout">
            Current demo assignment: <strong>Alangalang, Leyte</strong> · MD24 Community Clerkship · Preceptor: <strong>Dr. Angelita Jaya</strong>
          </div>
          <div className="list" style={{ marginTop: 14 }}>
            {orientationTasks.slice(0, 6).map(([task, isDone]) => (
              <div className="check-row" key={task}>
                <span className={isDone ? "check done" : "check"}>
                  <Icon name={isDone ? "check" : "rotation"} />
                </span>
                <span>{task}</span>
              </div>
            ))}
        ...
        <section className="panel panel-pad">
          ...
          <div className="list">
            {projects.slice(0, 3).map((project) => (
              <Link href={`/projects/${project.id}`} className="list-row" key={project.id}>
                <div className="list-main">
                  <div>
                    <strong>{project.title}</strong>
                    <span>{project.community} · {project.batch}</span>
                  </div>
                </div>
